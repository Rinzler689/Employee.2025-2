...
public class EmployeesUnitOfWork : GenericUnitOfWork<Employee>, IEmployeesUnitOfWork
{
    ...
    public EmployeesUnitOfWork(IGenericRepository<Employee> repository, IEmployeesRepository employeesRepository)
        : base(repository)
    {
    ...
    public async Task<ActionResponse<IEnumerable<Employee>>> GetByNameLastNameAsync(string search)
        => await _employeesRepository.GetByNameLastNameAsync(search);
}